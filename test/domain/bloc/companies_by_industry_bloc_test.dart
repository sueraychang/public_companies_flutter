...
import 'companies_by_industry_bloc_test.mocks.dart';

@GenerateMocks([DataRepository])
void main() {
  final repo = MockDataRepository();

  late CompaniesByIndustryBloc companiesByIndustryBloc;
  ...
  setUp(() {
    companiesByIndustryBloc = CompaniesByIndustryBloc(repo: repo);
  });

  group('CompaniesByIndustryBloc', () {
    test('Initial state is Loading', () {
      expect(companiesByIndustryBloc.state, isA<Loading>());
    });

    blocTest(
      'emits the Record of Industry and all the companies belong to it when the event GetCompanies is added',
      setUp: () {
        when(repo.getIndustry('01'))
            .thenAnswer((_) async => Result.success(testIndustries[0]));
        when(repo.getCompanies(forceUpdate: false))
            .thenAnswer((_) async => Result.success(testCompanies));
      },
      build: () => companiesByIndustryBloc,
      act: (bloc) => bloc.add(const GetCompanies('01')),
      verify: (bloc) {
        verify(repo.getIndustry('01')).called(1);
        verify(repo.getCompanies(forceUpdate: false)).called(1);

        final result = (bloc.state as Loaded).data;
        expect(result, isA<(Industry, List<Company>)>());

        expect(result.$1, testIndustries[0]);

        final companies = result.$2 as List<Company>;
        expect(companies.length, 2);
        expect(companies[0], testCompanies[0]);
        expect(companies[1], testCompanies[1]);
      },
      expect: () => [isA<Loaded>()],
    );

    blocTest(
      'emits error when failed to get the Industry',
      setUp: () {
        when(repo.getIndustry('01'))
      ...
      verify: (bloc) {
        verify(repo.getIndustry('01')).called(1);
        verifyNever(repo.getCompanies(forceUpdate: false));
    ...
    blocTest(
      'emits error when failed to get companies',
      setUp: () {
        when(repo.getIndustry('01'))
            .thenAnswer((_) async => Result.success(testIndustries[0]));
        when(repo.getCompanies(forceUpdate: false))
